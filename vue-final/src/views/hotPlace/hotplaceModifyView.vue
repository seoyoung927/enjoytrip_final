...
const handleSubmit = async () => {
  ...
  if (isValidate) {
    try {
      ...
      const response = await localFormAuth.patch("/hotplace", formDataToSend, {
        headers: {
...
const deleteHotplace = async () => {
  if (confirm("정말 삭제하시겠습니까?")) {
    try {
      const response = await localAuth.delete(`/hotplace/${hotplaceId}`);
...
const getHotplace = async () => {
  try {
    ...
    console.error("Error getting hotplace data:", error);
  }
};
  ...
  <div class="container">
    ...
    <div class="wrapper">
      <form class="form">
        ...
        <button @click="deleteHotplace" class="delete_btn" type="button">삭제</button>
      </form>
    </div>
  </div>
</template>
